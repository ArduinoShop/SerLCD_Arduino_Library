...
uint8_t SerLCD::getPos()
{
  uint8_t cnt = 0;
    ...
    while( Serial1.available() == 0 ) {
      cnt++;
     if( cnt > 250 ) {
         Serial.printf("SerLCD::getPos() Serial Timeout = %d\n", cnt );
        return 0xAA;
break;
        }
    }
      _pos = Serial1.read();
      Serial.printf("Pos = 0x%X\n", _pos );
if( _pos < 20) { Col = _pos; Row = 0; }
// if(Pos > 19) { Col = Pos%20; Row = 2; }
if( _pos > 63) { Col = _pos%64; Row = 1; }
// if(Pos > 83) { Col = Pos%84; Row = 3; }
// #ifdef DEBUG
//  Serial.printf("Cursor = %d, Col = %d, Row = %d\n", Pos, Col, Row );
// #endif  
      return _pos;
  } 
